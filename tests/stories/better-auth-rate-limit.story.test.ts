...
describe('Story: BetterAuth Rate Limiting', () => {
  ...
  describe('CoreBetterAuthRateLimiter Service', () => {
    ...
    describe('Disabled State', () => {
      ...
      it('should allow all requests when disabled', async () => {
        const result = await rateLimiter.check('192.168.1.1', '/sign-in');
    ...
    describe('Enabled State', () => {
      ...
      it('should use normal limits for non-strict endpoints', async () => {
        const ip = '192.168.1.102';

        // Non-strict endpoints get the full max (5)
        for (let i = 0; i < 5; i++) {
          const result = await rateLimiter.check(ip, '/profile');
          expect(result.allowed).toBe(true);
        }
      ...
      it('should skip rate limiting for skip endpoints', async () => {
        const ip = '192.168.1.103';

        // Session endpoint should skip rate limiting
        for (let i = 0; i < 100; i++) {
          const result = await rateLimiter.check(ip, '/session');
          expect(result.allowed).toBe(true);
          expect(result.limit).toBe(Infinity);
        }
      });
      ...
      it('should track different IPs independently', async () => {
        ...
        const result1 = await rateLimiter.check('192.168.1.1', '/profile');
        const result2 = await rateLimiter.check('192.168.1.2', '/profile');
      ...
      it('should track different endpoints independently for same IP', async () => {
        const ip = '192.168.1.104';

        // Make requests to different endpoints
        const result1 = await rateLimiter.check(ip, '/profile');
        const result2 = await rateLimiter.check(ip, '/settings');
    ...
    describe('Reset Functionality', () => {
      ...
      it('should reset rate limit for specific IP', async () => {
        ...
        // Make some requests
        await rateLimiter.check(ip, '/sign-in');
        await rateLimiter.check(ip, '/sign-in');
      ...
      it('should not affect other IPs when resetting one', async () => {
        ...
        await rateLimiter.check(ip1, '/sign-in');
        await rateLimiter.check(ip2, '/sign-in');

        // Reset only ip1
        await rateLimiter.reset(ip1);

        // ip1 should start fresh
        const result1 = await rateLimiter.check(ip1, '/sign-in');
        expect(result1.current).toBe(1);

        // ip2 should continue counting
        const result2 = await rateLimiter.check(ip2, '/sign-in');
        expect(result2.current).toBe(2);
      });

      it('should clear all entries', async () => {
        await rateLimiter.check('192.168.1.1', '/sign-in');
        await rateLimiter.check('192.168.1.2', '/sign-in');
        await rateLimiter.check('192.168.1.3', '/sign-in');
    ...
    describe('Statistics', () => {
      it('should return correct stats when disabled', async () => {
      ...
      it('should return correct stats when enabled', async () => {
        rateLimiter.configure({
          enabled: true,
          max: 10,
          windowSeconds: 60,
        ...
        await rateLimiter.check('192.168.1.1', '/test1');
        await rateLimiter.check('192.168.1.2', '/test2');
    ...
    describe('Reset Time Calculation', () => {
      ...
      it('should return correct resetIn value', async () => {
        const result = await rateLimiter.check('192.168.1.1', '/test');
    ...
    describe('Endpoint Normalization', () => {
      ...
      it('should skip callback endpoints', async () => {
        const result = await rateLimiter.check('192.168.1.1', '/callback/google');
      ...
      it('should handle query strings correctly', async () => {
        ...
        const result1 = await rateLimiter.check(ip, '/profile?foo=bar');
        const result2 = await rateLimiter.check(ip, '/profile?baz=qux');
  ...
  describe('Rate Limiting Configuration', () => {
    ...
    it('should have rate limit configuration in betterAuth config', async () => {
    ...
    it('should have message configuration', async () => {
      const config = mockConfigService.get('betterAuth');

      expect(config.rateLimit.message).toBe('Rate limited');
    });

    it('should have strictEndpoints configuration', async () => {
      const config = mockConfigService.get('betterAuth');

      expect(config.rateLimit.strictEndpoints).toContain('/sign-in');
    });

    it('should have skipEndpoints configuration', async () => {
      const config = mockConfigService.get('betterAuth');
  ...
  describe('Rate Limiter Module Integration', () => {
    it('should create CoreBetterAuthRateLimiter service', async () => {
    ...
    it('should be configurable after instantiation', async () => {
      const rateLimiter = new CoreBetterAuthRateLimiter();
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle IPv6 addresses', async () => {
      const result = await rateLimiter.check('::1', '/test');
    ...
    it('should handle long IP addresses', async () => {
      const result = await rateLimiter.check('2001:0db8:85a3:0000:0000:8a2e:0370:7334', '/test');

      expect(result.allowed).toBe(true);
    });

    it('should handle special characters in path', async () => {
      const result = await rateLimiter.check('192.168.1.1', '/test/path%20with%20spaces');
    ...
    it('should handle reconfiguration', async () => {
      ...
      await rateLimiter.check('192.168.1.1', '/test');
      await rateLimiter.check('192.168.1.1', '/test');
    ...
    it('should handle disabling after being enabled', async () => {
  ...
  describe('Default Configuration', () => {
    ...
    it('should use default skip endpoints', async () => {
      // /session should be skipped by default
      for (let i = 0; i < 100; i++) {
        const result = await rateLimiter.check('192.168.1.1', '/session');
        expect(result.allowed).toBe(true);
      }
    });

    it('should use default strict endpoints', async () => {
      const ip = '192.168.1.1';
