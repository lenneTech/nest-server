 ...
 * User service
 */
export abstract class CoreUserService<TUser = CoreUser, TUserInput = CoreUserInput, TUserCreateInput = CoreUserCreateInput> {
   ...
   * Create user
   */
  async create(input: TUserCreateInput): Promise<TUser> {
   ...
   * Get user via email
   */
  async getViaEmail(email: string): Promise<TUser> {
   ...
   */
  find(filterArgs?: FilterArgs): Promise<TUser[]> {
   ...
   * Get user via ID
   */
  async update(id: string, input: TUserInput): Promise<TUser> {
