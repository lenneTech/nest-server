...
export class CoreAiPromptBuilderService {
  ...
  protected buildToolProtocol(tools: IAiTool[]): string {
    ...
    return [
      ...
      'You may request multiple tools at once. After emitting tool_calls, STOP and output',
      'nothing else — do NOT write the results yourself. The system will send the real',
      'results back in a message starting with "TOOL_RESULTS:"; only then continue.',
      '',
