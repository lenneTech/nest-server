...
describe('CoreHubMailboxService', () => {
  ...
  it('returns false in copy mode (mail is still sent)', async () => {
    ...
    expect((await service.getMailbox()).mails).toHaveLength(1);
  });

  it('redacts tokens from the stored body in COPY mode (the real mail was still delivered)', async () => {
    ...
    const html = await service.getMailHtml(seq);
    expect(html).toBeDefined();
    // The captured audit copy must not expose a replayable reset token.
    expect(html).not.toContain('SUPERSECRETVALUE123');
  });

  it('keeps the raw body in CAPTURE mode (dev/test needs the clickable link; nothing was sent)', async () => {
    const service = new CoreHubMailboxService(makeConfig());
    const seq = service.captureAndGetSeq({
      html: '<a href="https://app.example.com/reset?token=SUPERSECRETVALUE123">Reset</a>',
      subject: 'Reset',
    ...
    expect(await service.getMailHtml(seq)).toContain('SUPERSECRETVALUE123');
  });

  it('evicts old mails beyond capacity (ring buffer)', async () => {
    const service = new CoreHubMailboxService(makeConfig());
  ...
  it('truncates oversized bodies to the per-mail cap', async () => {
    const service = new CoreHubMailboxService(makeConfig());
  ...
  it('serves stored HTML by seq for the preview iframe', async () => {
    const service = new CoreHubMailboxService(makeConfig());
    const seq = service.captureAndGetSeq({ html: '<p>Body</p>', subject: 'S', to: 'a@b.com' });
    expect(await service.getMailHtml(seq)).toContain('<p>Body</p>');
    expect(await service.getMailHtml(9999)).toBeUndefined();
  });

  it('clear() empties the mailbox', async () => {
    const service = new CoreHubMailboxService(makeConfig());
    service.capture({ subject: 'S', to: 'a@b.com' });
    service.clear();
    expect((await service.getMailbox()).mails).toHaveLength(0);
  });

  it('supports cursor-based polling via since', async () => {
    const service = new CoreHubMailboxService(makeConfig());
    service.capture({ subject: 'A', to: 'a@b.com' });
    const first = await service.getMailbox();
    service.capture({ subject: 'B', to: 'a@b.com' });
    const next = await service.getMailbox(first.cursor);
