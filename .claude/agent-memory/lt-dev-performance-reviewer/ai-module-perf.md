...
**Review gotcha — `OpenAiCompatibleProvider.detectCapabilities()` only probes UNDEFINED flags (stable, committed):**
It issues a probe for `supportsJsonResponse` / `supportsNativeTools` ONLY when that flag is `undefined` on the resolved connection; a DECLARED (boolean) flag is never re-probed (returns nothing for it). So any "compare DECLARED vs endpoint-actual" logic must FORCE a probe of its own — reusing `detectCapabilities()` on a connection that carries the declared booleans yields an empty result and cannot detect drift. When reviewing capability-drift / self-check features, verify they don't route through `detectCapabilities()` for already-declared flags (that path issues at most one probe — for the *undeclared* flag — whose result the drift comparison then discards). `resolve()` also does NOT populate `timeoutMs`, so probes always use the provider's 30s default regardless of a connection's `timeoutMs`.
- 2026-07-24 WIP (uncommitted on `develop`): `CoreAiConnectionService.warnOnCapabilityDrift()` (fire-and-forget in `onModuleInit`) hits exactly this trap — adds boot-time EXTERNAL probes + an N+1 (`find` then per-connection `resolve()` re-read) that produce no drift output. Under active review; re-verify against current code before relying on this note.
