...
## Verify "regenerated by build" without touching the tree (2026-09-26)

The generator hardcodes `ROOT = resolve(__dirname, '..')` and `OUTPUT = <ROOT>/FRAMEWORK-API.md`. To
check a working-tree FRAMEWORK-API.md is really generator output (not hand-edited) without
overwriting it: `sed` those two constants (ROOT → absolute repo path, OUTPUT → scratchpad) into a
temp copy under `scripts/`, run it with `npx tsx`, delete the copy, then `diff` — `IDENTICAL` settles it.
