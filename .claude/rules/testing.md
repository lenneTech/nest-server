...
pnpm run check:mutations -- --no-infra      # only the 67 that need no MongoDB
...
at 113 mutations — past the threshold this paragraph has carried since the gate was written,
...
spawn, transform, module graph, mongod connect, DB create and drop — once per mutation, 113 times.
...
recorded here rather than quietly absent, because one specific question is unanswered and a reader
should not have to infer it from silence:

> Under concurrent password-reset load, at what request rate does the added key-derivation work
> saturate the libuv threadpool?

The shape of the concern, for whoever picks this up: `crypto.scrypt` and node `bcrypt` both dispatch
to the libuv threadpool (default size **4**, shared with `fs`, `dns`, `zlib` and every other crypto
consumer in the process). Since 11.38.0 a single password reset occupies a slot for **two**
derivations rather than one — measured locally at ~76 ms (scrypt, N=16384/r=16) plus ~86 ms (bcrypt
cost 10). `/iam/request-password-reset` is unauthenticated, and the IP-axis `betterAuth.rateLimit`
is off unless configured. Threadpool exhaustion also does not present locally: four concurrent
resets stall every other threadpool consumer in the process, so the symptom appears far from the
cause.
