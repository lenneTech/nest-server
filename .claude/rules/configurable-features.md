...
| Central Redis | `redis` | Boolean Shorthand + Presence Implies Enabled | `host: 'localhost'`, `port: 6379`, `db: 0`, `keyPrefix: 'nest-server'`; `url` (takes precedence over host/port/db/credentials), `username`/`password`, `options` (passed to the ioredis constructor). `true` / `{}` enables with defaults; `{ enabled: false }` pre-configures without enabling; **absent = every consumer keeps its process-local fallback** (that fallback is the whole backward-compatibility story of 11.33.0). Requires the OPTIONAL peer `ioredis` — configured-but-missing **fails the boot** with a named error rather than crashing on first use. `keyPrefix` is applied by the framework per key, NOT as ioredis `keyPrefix` (that would collide with BullMQ's own prefix). One `CoreRedisService` serves all features: shared client (`getClient()`), one cached subscriber (`getSubscriber()` — a subscribing client cannot run commands), dedicated connections (`createClient(label)`); all are tracked and quit on shutdown. Switches on automatically: exact cross-replica rate limits via `RedisRateLimitStore` (see the Legacy Auth / BetterAuth / AI rate-limit rows), cron dedup (see Cron Job Deduplication row), `CoreRedisPubSub` as `PUB_SUB` for cluster-wide GraphQL subscriptions (**payloads must be JSON-serializable — `Date`, class instances, `Map`/`Set`, `undefined` do not survive**), tenant-cache invalidation broadcast, Hub collector mirroring, MCP session registry (turns a wrong-replica request from a misleading `404` into a `409` — `/ai/mcp` still REQUIRES sticky sessions, sessions are not portable) |
| S3 File Storage | `s3` + `fileStorage` | Presence Implies Enabled (`s3`) + Explicit Enum (`fileStorage`) | `fileStorage: 'gridfs'` (default) \| `'s3'`. `s3`: `bucket` (required), `region: 'us-east-1'`, `forcePathStyle: false`, `stagingBucket: bucket`, `presignedDownloads: false` (`true`/`{}` → `expiresInSeconds: 300`), `endpoint` (MinIO/RustFS), `accessKeyId`/`secretAccessKey` (omit → AWS default credential chain), `enabled: false` to pre-configure. **Two separate switches:** `s3` alone only makes `CoreS3Service` available (TUS staging, own code); files move only when `fileStorage: 's3'` is ALSO set. Requires the OPTIONAL peer `@aws-sdk/client-s3` (configured-but-missing **fails the boot**), plus `@aws-sdk/s3-request-presigner` for presigned downloads. **Forward-only, no cut-over:** reads check the `s3-files` metadata collection first and fall back to GridFS, so pre-existing GridFS files stay readable and `getFileInfos()` returns the union. Metadata lives in the schema-less `s3-files` collection (GridFS's `fs.files` equivalent); the object key is the string form of `_id`. `presignedDownloads` makes `GET /files/id/:id` answer `302` to a time-limited S3 URL instead of streaming — deliberately **fail-soft**: missing presigner peer, S3 outage, or a project `FileService` predating `getDownloadUrl()` all fall through to streaming (which runs the same rights check and never confirms a file exists). A project `FileService` must forward the deps — `super(connection, 'fs', { configService, s3Service })` — otherwise `fileStorage: 's3'` has no effect on it |
...
| Shutdown Delay | `shutdownDelayMs` | Numeric Sentinel — Family B (`0` = off) | `0` (default, no delay, no log). Delays `beforeApplicationShutdown` by N ms so a load balancer can finish deregistering the instance before in-flight connections are drained — the hook runs BEFORE `onApplicationShutdown`, where the framework closes Redis/Mongo. **Requires `server.enableShutdownHooks()` in main.ts** (present in the starter); without it the hook never fires and the option silently does nothing. Non-numeric / negative values behave like `0` |
