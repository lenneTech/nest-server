...
| AI Assistant | `ai` | Presence Implies Enabled | `maxIterations: 5`, `rateLimit` (presence implies enabled: `max: 20`, `windowSeconds: 60`), `audit: false` (persist runs to `aiInteractions`, admin-readable), `mcp: false` (expose registry as MCP server at `/ai/mcp`, Bearer-auth, lazy `@modelcontextprotocol/sdk`). DB-backed LLM connections (`aiConnections`, admin CRUD) with AES-256-GCM-encrypted API keys (`AiCryptoService`, secret from `ai.encryptionSecret` / `NSC__AI__ENCRYPTION_SECRET` / `SECRETS_ENCRYPTION_KEY`; `apiKeyEncrypted` is a global `secretFields` entry, never returned — only `hasApiKey`). Provider abstraction (`ILlmProvider`, default `OpenAiCompatibleProvider` for mittwald/Ollama via `fetch`; emulated tool calling when `supportsNativeTools` is false). Global `AiToolRegistry` (tools self-register, role-filtered; tools may be `destructive` → require `confirm: true`). Multi-turn conversations (`aiConversations`, owner-scoped). SSE streaming via `POST /ai/stream`. Optional `defaultConnection` one-time seed. Overrides via `CoreModule.forRoot(env, { ai: { service, promptBuilder, connectionService, conversationService, interactionService, resolver, controller } })` |
