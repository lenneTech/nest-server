...
| Offline Sync | `sync` | Boolean Shorthand | `enabled: false` (off by default). When enabled (`true` / `{}`): adds `mongooseSyncPlugin` to all schemas (no-op on schemas without `syncable: true`), exposes REST `GET/POST /sync/:model` (default `S_USER`, rate-limited 60/30 per minute), GraphQL subscription `syncHint` with debounce and per-tenant filter. Per-leg toggles `pull.enabled` / `push.enabled` / `hint.enabled`. Per-model entries (`models[]`) require `service`, `createDto`, `updateDto`. Idempotency cache (TTL 86400s, configurable). Override `pubSub` and `rateLimitGuard` for multi-instance. Hard delete is unreachable through sync push (forced false). See [`src/core/modules/sync/`](../../src/core/modules/sync/) for README, INTEGRATION-CHECKLIST, SECURITY |
