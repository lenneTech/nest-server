...
| Category | Details |
|----------|---------|
| **Breaking Changes** | Cookie structure simplified, `better-auth.session_token` removed, `termsAndPrivacyAccepted` required by default |
| **New Features** | Auto AppName detection, Token analysis helpers, Cookie-Mode/JWT-Mode dual support, Email Verification, Sign-Up Checks |
| **Bugfixes** | Passkey/2FA now works correctly in both Cookie and JWT modes |
| **Migration Effort** | Low (~15 minutes) - Cookie access changes + optional sign-up flow updates |
...
### 5. Email Verification (enabled by default)

Email verification via Better-Auth's `emailVerification` plugin is now **enabled by default**:
...
// To restore old behavior (disable email verification):
betterAuth: {
...
### 6. Sign-Up Checks (enabled by default)
...
```typescript
// To restore old behavior (no sign-up checks):
betterAuth: {
  signUpChecks: false,
}
...
### 4. Sign-Up Now Requires `termsAndPrivacyAccepted`
...
### Step 4: Update Sign-Up Calls (if applicable)
...
### Step 5: Update Custom Resolvers and Controllers (if applicable)
...
- [ ] 2FA Enable/Verify
- [ ] Sign-Out
- [ ] Sign-Up with `termsAndPrivacyAccepted: true`
- [ ] Sign-Up without `termsAndPrivacyAccepted` (should fail with error LTNS_0021)
- [ ] Email verification link in console (local mode)
- [ ] `verifiedAt` set after email verification
...
```typescript
// Token analysis helpers (11.12.1)
export {
  analyzeToken,
  getUserIdFromToken,
  isBetterAuthJwt,
...
// Email Verification & Sign-Up Checks (11.12.1)
