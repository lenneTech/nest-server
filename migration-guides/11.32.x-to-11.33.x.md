...
| **Under the hood (§14)** | Two new MongoDB collections (`s3-files`, `filesystem-files` — include them in backups), new public exports, a bucket-less `s3` block is now ignored, NestJS 11.1.28 → 11.1.29 |
...
call. `getRawFileInfo()` checks S3 metadata first, then GridFS — the same order as `getFileInfo()`,
so a rule sees the file the download would actually serve (§7).
...
`file.storage: 's3'` alone has no effect on a `FileService` that does not forward the dependencies.
...
- **The limit is only as exact as the client IP.** Both rate-limit entry points take the first
  entry of `X-Forwarded-For` when present, and trust it unconditionally — there is no `trustProxy`
  allow-list. That predates this release, but it becomes load-bearing here: the whole point of the
  shared counter is an exact limit, and a caller who sets that header per request is counted as a
  new client every time. **Terminate `X-Forwarded-For` at your reverse proxy** — overwrite it rather
  than appending — so only the proxy can set it. Without a proxy in front, do not expose the app
  directly; the header is attacker-controlled then.
...
import {
  ...
  CoreRedisService, CoreS3Service, CoreRedisPubSub, CoreCronJobsRegistry,
...
| `File storage 's3' was selected automatically (s3.bucket is configured) but is not available` | Same, but the driver was DERIVED: an `s3` block makes S3 the default | If S3 is only meant for tus staging, say so: `file: { storage: 'gridfs' }` — §7 |
