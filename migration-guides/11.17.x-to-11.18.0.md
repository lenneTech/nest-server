...
| **Bugfixes** | None |
| **Migration Effort** | ~5 minutes (update + verify) |

---

## Quick Migration (No Breaking Changes)

All new features are **enabled by default** and fully backward compatible. No code changes required.

```bash
# Update package
npm install @lenne.tech/nest-server@11.18.0
...
### Custom roles cannot assign roles to other users

By default, only ADMIN can assign roles. Configure `allowedRoles` to permit additional roles:
